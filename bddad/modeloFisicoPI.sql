...
codInterno integer,
codMorada integer,
    constraint pk_CodInterno_CodMorada PRIMARYT KEY (codInterno,codMorada)
);
alter table MoradaCorrespondencia add constraint fk_moradaCorrespondencia_CodInterno FOREIGN KEY (codInterno) references Clientes(codInterno);
alter table MoradaCorrespondencia add constraint fk_moradaCorrespondencia_CodMorada FOREIGN KEY (codMorada) references Moradas(codMorada);
...
alter table Cliente_Niveis add constraint fk_clienteNivel_CodInterno FOREIGN KEY (codInterno) references Clientes(codInterno);
alter table Cliente_Niveis add constraint fk_clienteNivel_CodNivel FOREIGN KEY (codNivel) references Nivel (codNivel);
...
codColheita integer,
anoColheita integer nnAnoColheita NOT NULL,
    constraint pk_codColheita PRIMARY KEY (codColheita)
);
create table Culturas(
codCultura integer,
designacaoCultura varchar(50) nnDesignacaoCultura NOT NULL,
...
alter table AplicacaoRegas add constraint fk_aplicacaoRegas_CodSetorAgricola FOREIGN KEY (codSetorAgricula) references SetoresAgricolas (codSetorAgricola);
...
alter table Incidentes add constraint fk_IncidentescodPagamento FOREIGN KEY (codPagamento) references Pagamento(codPagamento);
...
alter table Produtos add constraint fk_Produtos_CodCultura FOREIGN KEY (codCultura) references Cultura (codCultura);
alter table Produtos add constraint fk_Produtos_CodColheita FOREIGN KEY (codColheita) references Colheita(codColheita);
...
alter table Pedidos_Pagamentos_Entregas add constraint fk_Pedidos_Pagamentos_Entregas_CodPagamento FOREIGN KEY (codPagamento) references Pagamentos (codProduto);
...
alter table Operacoes add constraint fk_Operacoes_codCalendarioOperacoes FOREIGN KEY (codCalendarioOperacoes) references CalendarioOperacoes (codCalendarioOperacoes);
...
alter table PlanosRega add constraint fk_PlanosRega_codCultura FOREIGN KEY (codCultura) references Culturas (codCulturas);
...
periodicidadeRegaSetor integer nnPeriodicidadeRegaSetor NOT NULL,
...
alter table AplicacaoFatorProducao add constraint fk_AplicacaoFatorProducao_codFatorProducao FOREIGN KEY (codFatorProducao) references FatoresProducao (codFatoresProducao);
...
alter table Restricoes add constraint fk_Restricoes_codZonaGeografica FOREIGN KEY (codZonaGeografica) references ZonaGeografica (codZonaGeografica);
...
alter table EstacoesMeteorologicas add constraint fk_EstacoesMeteorologicas_codZonaGeografica FOREIGN KEY references ZonaGeografica (codZonaGeografica);
...
codLeitura integer,
idSensor varchar(5),
instanteLeitura date nnInstanteLeitura NOT NULL,
valorLido integer nnValorLido NOT NULL,
    constraint pk_codLeitura PRIMARY KEY (codLeitura)
);
alter table Leitura add constraint fk_Leitura_idSensor FOREIGN KEY (idSensor) references Sensores (idSensor);
alter table Leitura add constraint ck_valorLido CHECK(valorLido>0 && valorLido<100);
